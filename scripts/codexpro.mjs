...
import { ask, labelValue, paint, printBox, statusLine } from './cli-ui.mjs';
import { runExecuteHandoff, runWatchHandoff } from './handoff-execution.mjs';
import {
  chooseReusableProfile,
  codexProHome,
...
async function main() {
  ...
  if (subcommand === 'execute-handoff' || subcommand === 'execute' || subcommand === 'run-handoff') {
    const handoffArgs = parseArgs(argv.slice(1));
    if (handoffArgs.help) {
      usage();
      return;
    }
    handoffArgs.root = realDir(handoffArgs.root ?? process.env.CODEXPRO_ROOT ?? process.cwd());
    await runExecuteHandoff(handoffArgs, { commandAvailableFromRoot });
    return;
  }
  if (subcommand === 'watch-handoff' || subcommand === 'watch') {
    ...
    handoffArgs.root = realDir(handoffArgs.root ?? process.env.CODEXPRO_ROOT ?? process.cwd());
    await runWatchHandoff(handoffArgs, { commandAvailableFromRoot });
    return;
  }
